...
import java.util.List;
    ...
    public class MembershipFreezeServiceImpl implements MembershipFreezeService {
        ...
        public AdminFreezeRequestResponse approveFreezeRequest(Long requestId, User reviewer) {
            ...
            return toAdminResponse(freezeRequest);
        }
        ...
        private void validateFreezeRequest(Membership membership, Long membershipId, SubmitFreezeRequest req) {
            ...
            if (ChronoUnit.DAYS.between(req.freezeStart(), req.freezeEnd())> ApplicationConstants.MAX_FREEZE_DAYS)
        ...
        private void applyFreeze(MembershipFreezeRequests freezeRequest) {
            long freezeDays = ChronoUnit.DAYS.between(freezeRequest.getFreezeStart(), freezeRequest.getFreezeEnd());
        ...
        private UserFreezeRequestResponse toUserResponse(MembershipFreezeRequests freezeRequest) {
            Membership membership = freezeRequest.getMembership();
            return new UserFreezeRequestResponse(
                    freezeRequest.getId(), membership.getId(), membership.getMembershipType().getName(),
                    membership.getStartDate(), membership.getEndDate(), membership.getStatus(),
                    freezeRequest.getFreezeStart(), freezeRequest.getFreezeEnd(), freezeRequest.getStatus(),
                    freezeRequest.getCreatedAt(), freezeRequest.getReviewedAt(), freezeRequest.getRejectionReason());
        }

        private AdminFreezeRequestResponse toAdminResponse(MembershipFreezeRequests freezeRequest) {
            Membership membership = freezeRequest.getMembership();
            User user = membership.getUser();
            return new AdminFreezeRequestResponse(
                    freezeRequest.getId(), membership.getId(), membership.getMembershipType().getName(),
                    user.getId(), user.getFirstName(), user.getLastName(), user.getEmail(),
                    freezeRequest.getFreezeStart(), freezeRequest.getFreezeEnd(), freezeRequest.getStatus(),
                    freezeRequest.getCreatedAt(),
                    freezeRequest.getReviewedBy() != null ? freezeRequest.getReviewedBy().getId() : null,
                    freezeRequest.getReviewedAt(), freezeRequest.getRejectionReason());
        }
    }